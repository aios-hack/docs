...
[ScheduleFeatureizer](backend/contexts/surrogate/domain/features.py) строит признаки из расписания и доступной истории. Фактический будущий отклик OPM кандидата на инференсе недоступен и в признаки не подставляется.
...
Код: [model.py](backend/contexts/surrogate/application/model.py), [ensemble.py](backend/contexts/surrogate/application/ensemble.py), [adapter.py](backend/contexts/surrogate/application/adapter.py).
...
Код: [npv_block_head.py](backend/contexts/surrogate/domain/npv_block_head.py), [npv_economic_features.py](backend/contexts/surrogate/domain/npv_economic_features.py), [schedule_search.py](backend/contexts/optimization/application/environment.py).
...
**Оговорка, без которой предыдущий абзац вводит в заблуждение:** ранговое дообучение — это
свойство тренера и пробных обучений 06.09, **а не действующих production-весов**. Действующие
веса обучены Smooth L1 при `ranking_loss_weight = 0` с выбором эпохи по `loss`. Разделение по
трём колонкам — в §11.4.
...
Весь этот раздел описывает **возможность тренера и результат пробных обучений 06.09**. Он не
описывает действующие production-веса: у них `ranking_loss_weight = 0`, ранговый член в их
обучении не участвовал. См. §11.4.
...
### 11.4. Тренер, эксперимент, production — три разные вещи

Самый частый способ ошибиться на защите — сказать «модель обучена с ранговым членом и учитывает
измеренный лаг». Код это опровергает. Ниже три колонки строго разделены: что **умеет тренер**
(возможности кода), что **проверено экспериментом** (пробные обучения 06.09, §11.2), и какие
свойства у **действующих весов**, которые стоят в production и дают числа §11.1.

| Свойство | Умеет тренер (код) | Проверено экспериментом 06.09 | Действующие веса production |
|---|---|---|---|
| Ранговый член в лоссе | да, параметр `ranking_loss_weight` в `Settings` (`backend/contexts/surrogate/application/model.py`); при значении `> 0` включается попарный ранговый член | да, `ranking weight 3`, три пробных обучения (§11.2) | **нет: `ranking_loss_weight = 0`.** Обучение только Smooth L1 |
| Критерий выбора лучшей эпохи | три варианта `select_by`: `loss`, `money`, `rank` (там же, `_SELECTION_CRITERIA`) | `rank` — по фактическому ЧДД validation | **`loss`** — значение по умолчанию |
| Параметризация цели | два варианта `target_parameterization`: `absolute`, `watercut` (`TARGET_PARAMETERIZATIONS`) | `absolute` | **`absolute`** |
| Точные метки ЧДД (`exact-npv`) | да, режим реализован | да, дал основной выигрыш опыта | нет: веса старше этого режима |
| Сводка сценария на узле | `scenario_context`: `False`, `True`, `"mean"`, `"rich"` | `"mean"` | **`False`** |
| Измеренный лаг λ в признаках | **нет.** `lag_months` есть в контракте `Lambda` (`backend/contexts/connectivity/domain/connectivity.py`), но `ScheduleFeatureizer` его не читает: в признаки идут только `matrix`, `window_start`, `window_end` (`backend/contexts/surrogate/domain/features/`) | не проверялось: признака нет | **нет** |
| Рёбра λ на входе сети | структура `LambdaEdgeFeature` строится featureizer'ом | — | **нет: `lambda_edges` обнуляются** перед каждым вызовом модели — в поиске (`backend/contexts/optimization/application/environment.py`), в тренере (`backend/contexts/surrogate/application/train/`), в проверке (`backend/interfaces/cli/surrogate/check.py`) и в отчёте метрик |
| Влияние соседей | — | — | **есть, но в свёрнутом виде:** два числа на узел — `cumulative_neighbor_injection_m3` и `current_neighbor_injection_m3_per_day`, это суммы `коэффициент λ × закачка соседа` за тот же шаг |

Что из этой таблицы произносится вслух:

- **«Действующие веса обучены без рангового члена»** — прямой ответ, и он не ослабляет позицию:
  ранговое качество §11.1 (Spearman `0.8602`, precision@5 `0.80`) достигнуто блендом с
  отдельной экономической головой, а не ранговым лоссом траекторной сети. Ранговое обучение —
  проверенный, но ещё не принятый в production путь.
- **«Лаг λ модель не видит»** — в признаки идёт коэффициент влияния текущего шага, сдвига во
  времени нет. На действующем окне 2007–2025 измеренный лаг и так равен нулю (`FAQ.md` §3),
  поэтому потери информации на этом окне нет; на окне, где лаг ненулевой, признак пришлось бы
  добавлять. Это ограничение, а не скрытая ошибка.
- **«Полный измеренный граф связности нейросети не подаётся»** — рёбра сворачиваются в две
  агрегированные величины на узел, а список рёбер обнуляется. Заявлять, что суррогат работает по
  полной матрице λ, нельзя.
- **Ни один экспериментальный результат §11.2 не перенесён в production.** Веса не переобучены;
  это зафиксировано в `RELEASE_SURROGATE_20260906.md`.
...
aios run search --run-id defence-search
aios run verify --run-id defence-search
```
...
```bash
.venv/bin/python -m backend.interfaces.cli.surrogate.tools.surrogate_metrics_report \
  --output out/metrics-full-pipeline.json

.venv/bin/python -m backend.interfaces.cli.surrogate.tools.surrogate_train_tensors \
...
**Ваша модель обучена с ранговым членом?** Нет. Тренер это умеет, эксперимент 06.09 это проверил, но **действующие веса обучены при `ranking_loss_weight = 0`** с выбором эпохи по `loss` и параметризацией `absolute`. Ранговое качество §11.1 даёт бленд с отдельной экономической головой, а не ранговый лосс. Полное разделение — §11.4.

**Модель учитывает измеренный лаг связности?** Нет. В признаки идёт коэффициент λ текущего шага, сдвига по времени нет: `lag_months` есть в контракте `Lambda`, но `ScheduleFeatureizer` его не читает. На действующем окне 2007–2025 измеренный лаг равен нулю, поэтому на нём потери нет; на окне с ненулевым лагом признак пришлось бы добавить.

**Сеть работает по полной матрице связности?** Нет. Список рёбер `lambda_edges` обнуляется перед каждым вызовом модели; влияние соседей входит двумя агрегированными числами на узел.
...
`aios web`. Это не публичный многопользовательский сервис:
