...
| 23.08 | Датированные снимки состояния лежали отдельными файлами в корнях `docs/v2/` и `aios/`; при следующей передаче создавался новый файл или переименование в соседней ветке | Снимок состояния — это датированная находка в `v2/findings/` либо строка в указателе на живущий документ; отдельный снимок в корне не заводится. Живущий `SURROGATE_HANDOFF.md` остаётся один и правится на месте | Снимки дублировали статус и handoff, а переименование `SURROGATE-REQUESTS-20.08.md` в соседней ветке уже создало конфликт. Находки сохраняют исторический замер, указатель — единственный актуальный маршрут чтения. Задача D6 |
